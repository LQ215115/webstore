...
public class AdminAddFoodServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		sfu.setFileSizeMax(500 * 1024);//设置单个上传的文件上限为500KB
		/*
		...
		try {
			...
			error(sid,"上传的文件超出了80KB", request, response);
			return;
		}
		...
		Food food = CommonUtils.toBean(map, Food.class);//把Map中大部分数据封装到Book对象中
		...
		if(!filename.toLowerCase().endsWith(".jpg")&&!filename.toLowerCase().endsWith(".png")&&!filename.toLowerCase().endsWith(".jpeg")) {
			error(sid,"上传的图片扩展名必须是JPG,PNG,JPEG", request, response);
			return;
		}
		// 校验图片的尺寸
		 ...
		 * 1. 获取真实路径
		 */
		String savepath = this.getServletContext().getRealPath("/foodimages");
		/*
		 * 2. 创建目标文件
		 */
		...
		if(image.getWidth(null) > 200 || image.getHeight(null) > 200) {
			error(sid,"您上传的图片尺寸超出了200*200！", request, response);
			destFile.delete();//删除图片
			return;
		}
		
		// 把图片的路径设置给book对象
		food.setImg("foodimages/" + filename);
		
		// 调用service完成保存
		food.setFid(CommonUtils.uuid());
		FoodService bookService = new FoodService();
		bookService.add(food);
		
		// 保存成功信息转发到msg.jsp
		request.setAttribute("msg", "添加食品成功！");
	 ...
	 * 保存错误信息，转发到add.jsp
	 */
	private void error(String sid,String msg, HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		request.setAttribute("msg", msg);
		request.setAttribute("parents", new CategoryService().findParents(sid));//所有一级分类
		request.getRequestDispatcher("/admin/jsp/addfood.jsp").
