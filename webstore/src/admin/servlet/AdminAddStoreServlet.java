...
public class AdminAddStoreServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Store store = CommonUtils.toBean(map, Store.class);//把Map中大部分数据封装到Book对象中
		Place place = CommonUtils.toBean(map, Place.class);//把Map中cid封装到Category中
		...
		if(!filename.toLowerCase().endsWith(".jpg")&&!filename.toLowerCase().endsWith(".png")&&!filename.toLowerCase().endsWith(".jpeg")) {
			error("上传的图片扩展名必须是JPG,PNG,JPEG", request, response);
			return;
		}
		// 校验图片的尺寸
		 ...
		 * 1. 获取真实路径
		 */
		String savepath = this.getServletContext().getRealPath("/storeimages");
		/*
		 * 2. 创建目标文件
		 */
		...
		if(image.getWidth(null) > 200 || image.getHeight(null) > 200) {
			error("您上传的图片尺寸超出了200*200！", request, response);
			destFile.delete();//删除图片
			return;
		}
		
		// 把图片的路径设置给book对象
		store.setImg("storeimages/" + filename);
		
		// 调用service完成保存
		store.setId(CommonUtils.uuid());
		StoreService storeService = new StoreService();
		storeService.add(store);
		
		// 保存成功信息转发到msg.jsp
		request.setAttribute("msg", "添加店铺成功！");
	...
	private void error(String msg, HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("/admin/jsp/addstore.jsp").
