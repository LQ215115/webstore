...
public class OrderDao {
	...
	public void add(Order order) throws SQLException {
		...
		for(int i = 0; i < len; i++){
			...
			objs[i] = new Object[]{item.getOrderItemId(),item.getQuantity(),
					item.getSubtotal(),item.getFood().getFid(),
					item.getFood().getFname(),item.getFood().getCurrPrice(),
					item.getFood().getImg(),order.getOid()};
		}
		qr.batch(sql, objs);
	}
	...
	public PageBean<Order> findByStatus(int status, int pc) throws SQLException {
		List<Expression> exprList = new ArrayList<Expression>();
		exprList.add(new Expression("status", "=", status+""));//要求字符串参数
		return findByCriteria(exprList, pc);
	}
	/**
	 * 查询所有订单
	...
	public Double getTotal(String oid) throws SQLException {
		String sql = "select total from t_order where oid=?";
